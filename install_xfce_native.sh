...
# Set aliases
echo "
alias debian='proot-distro login debian-oldstable --user $username --shared-tmp'
...
file:///data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username debian-oldstable Home
...
#!/bin/bash
varname=$(basename $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/*)
pd login debian-oldstable --user $varname --shared-tmp -- env DISPLAY=:0 $@
...
#!/bin/bash
varname=$(basename $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/*)
pd login debian-oldstable --user $varname --shared-tmp -- env DISPLAY=:0 MESA_LOADER_DRIVER_OVERRIDE=zink TU_DEBUG=noconform $@
...
#!/bin/bash
varname=$(basename $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/*)
pd login debian-oldstable --user $varname --shared-tmp -- env DISPLAY=:0 MESA_LOADER_DRIVER_OVERRIDE=zink TU_DEBUG=noconform GALLIUM_HUD=fps $@
...
# Install Debian proot
pd install debian-oldstable
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 apt update
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 apt upgrade -y
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 apt install "${pkgs_proot[@]}" -y -o Dpkg::Options::="--force-confold"

# Create user
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 groupadd storage
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 groupadd wheel
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 useradd -m -g users -G wheel,audio,video,storage -s /bin/bash "$username"

# Add user to sudoers
chmod u+rw $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/etc/sudoers
echo "$username ALL=(ALL) NOPASSWD:ALL" | tee -a $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/etc/sudoers > /dev/null
chmod u-w  $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/etc/sudoers

# Set proot DISPLAY
echo "export DISPLAY=:0" >> $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.bashrc
...
eval "$(starship init bash)"
" >> $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.bashrc

# Set proot timezone
timezone=$(getprop persist.sys.timezone)
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 rm /etc/localtime
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 cp /usr/share/zoneinfo/$timezone /etc/localtime

# Setup Hardware Acceleration in proot
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 wget https://github.com/phoenixbyrd/Termux_XFCE/raw/main/mesa-vulkan-kgsl_24.1.0-devel-20240120_arm64.deb
pd login debian-oldstable --shared-tmp -- env DISPLAY=:0 sudo apt install -y ./mesa-vulkan-kgsl_24.1.0-devel-20240120_arm64.deb

mkdir -p $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.config/

# Download proot starship theme
curl -o $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.config/starship.toml https://raw.githubusercontent.com/phoenixbyrd/Termux_XFCE/refs/heads/main/starship_proot.toml
sed -i "s/phoenixbyrd/$username/" $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.config/starship.toml

wget https://github.com/phoenixbyrd/Termux_XFCE/raw/main/conky.tar.gz
tar -xvzf conky.tar.gz
rm conky.tar.gz
mv $HOME/.config/conky/ $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/home/$username/.config/

# Conky
cp $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/usr/share/applications/conky.desktop $HOME/.config/autostart/
sed -i 's|^Exec=.*$|Exec=prun conky -c .config/conky/Alterf/Alterf.conf|' $HOME/.config/autostart/conky.desktop

# Flameshot
cp $PREFIX/var/lib/proot-distro/installed-rootfs/debian-oldstable/usr/share/applications/org.flameshot.Flameshot.desktop $HOME/.config/autostart/
